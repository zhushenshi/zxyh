...
export default {
  ...
  methods:{
    ...
    getMegIDCardQualitySuccess (data) {//身份证识别成功
      let _this=this
      if(typeof(data)==='object'){
        _this.$ajax('post','/upload/uploadFile',{fileBase64:"data:image/png;base64,"+data.image},function(info){
          if(_this.type==='0'){//人像面
          // alert(data.name)
            // this.form.icCardImg1="data:image/png;base64,"+msg.image
            _this.form.icCardImg1=info.retData
            _this.form.custName=data.name  //姓名，
            _this.form.cardNum=data.idcard //身份证号码
            _this.form.sex=data.sex //性别
            _this.form.nation=data.nation //民族
            _this.form.birthday=data.birth//出生日期
            _this.form.cardAddress=data.address//身份证证件地址
            _this.scanFront=true//人像面已扫描
          }else if(_this.type==='1'){
            // this.form.icCardImg2="data:image/png;base64,"+msg.image
            _this.form.icCardImg2=info.retData
            _this.form.creditIssueOrg =data.office//签发机关
            _this.form.cardEffectedDate =data.validDate//身份证有效期
    ...
    getRealNameInfo(){
      ...
      This.$ajax('post','/customer/getCustomerCardInfo','',function(data){
        if(data.retData.custName){
          ...
          var list=customer.videoList
          list.forEach(element => {
            if(element.frontBack == '1'){
              This.form.icCardImg1 = element.path
            }else if(element.frontBack == '2'){
              This.form.icCardImg2 = element.path
            }
          });
          // if(customer.videoList[0].frontBack == '2'){
          //   This.form.icCardImg2=customer.videoList[0].path
          //   This.form.icCardImg1=customer.videoList[1].path
          // }else{
    ...
    callHander (type) {
      ...
      this.type=type
      this.$ajax('post','/ocr/getNonceTicket','',(data)=>{
        if(data.retData){
          this.getOcrFun([data.retData.sign,data.retData.nonceStr,data.retData.userId,type,data.retData.app_id])
        }
      })
    },
    getOcrFun (option){
      let me=this
      let phones = navigator.userAgent.toLowerCase()
      if (/iphone|ipad|ipod/.test(phones)) {
        try{
          if(device&&typeof(device.getTencentOCR)=="function"){
            device.getTencentOCR(me.getMegIDCardQualitySuccess,me.getMegIDCardQualityError,option);
          }else{}
        }catch(e){}
      }else if(/android/.test(phones)){
        try{
          if(device&&typeof(device.cloudOCR)=="function"){
            device.cloudOCR(me.getMegIDCardQualitySuccess,me.getMegIDCardQualityError,option);
          }else{}
        }catch(e){}
      }
    },
